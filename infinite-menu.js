/**
 * Infinite Menu (React Bits) - vanilla port.
 * Code kept as-is; runs when #pageInfiniteMenu has class .open.
 */
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (this.snapTargetDirection) {
        ...
        const sqrDist = vec3.sqrDist(a, b);
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    // alpha: true so backgrounds behind the canvas (pixel snow) can show through
  ...
  #initTexture() {
    ...
    if (this.items.length === 0) {
      ...
      ctx.fillText('No images', canvas.width / 2, canvas.height / 2);
      gl.bindTexture(gl.TEXTURE_2D, this.tex);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas);
      gl.generateMipmap(gl.TEXTURE_2D);
      return Promise.resolve();
    }
    return Promise.all(this.items.map((item, i) => new Promise((resolve) => {
      const img = new Image();
      ...
      img.onload = () => {
        ...
        const scale = Math.min(cellSize / iw, cellSize / ih, 1);
        const dw = iw * scale;
        const dh = ih * scale;
        const isMuse = (item.image && item.image.toLowerCase().includes('muse')) || (item.title && item.title.toLowerCase() === 'muse');
        if (isMuse) {
          ctx.save();
          ...
          ctx.drawImage(img, 0, 0, iw, ih, (cellSize - dw) / 2, (cellSize - dh) / 2, dw, dh);
          ctx.restore();
        } else {
...
const defaultItems = [
  { image: 'child.jpeg', title: 'child', description: 'child took some insane pics' },
  { image: 'kiss.jpeg', title: 'suits n sarrees', description: 'this day was so cute' },
  { image: 'angie.jpeg', title: 'studying with u', description: 'studying with u was da best' },
  { image: 'couchpotato.jpeg', title: 'the bean bag', description: 'this beanbag was so comfy.' },
  { image: 'moobean.jpeg', title: 'moo and bean', description: 'moo and bean are so cute 🥹' },
  { image: 'pic.jpeg', title: 'cute', description: 'this is one of my fav pics of us lub' },
  { image: 'sun.jpeg', title: 'sunkissed', description: 'you look so cute my lub' },
  { image: 'boat.jpeg', title: 'boating was fun', description: 'had my dawgs out and in da water' },
  { image: 'leap.jpeg', title: 'amazing date', description: 'this was an amazing date (+ vv, + puta). the only other gorgeous person or thing in my radius than the leaves was you my love' },
  { image: 'dinner.jpeg', title: 'dinner', description: 'this date was amazing, minus the comin back part' },
  { image: 'muse.jpeg', title: 'museum', description: 'we should go back there frfr.' },
  { image: 'Cathedral.jpeg', title: 'the cathedral', description: 'that day was goddamn crazy.' }
];

(function () {
  const page = document.getElementById('pageInfiniteMenu');
  const canvas = document.getElementById('infinite-grid-menu-canvas');
  const titleEl = document.getElementById('infinite-menu-title');
  const descEl = document.getElementById('infinite-menu-description');
  if (!page || !canvas) return;

  let sketch = null;
  let resizeHandler = null;

  function updateOverlay(activeItem, isMoving) {
    const active = activeItem && !isMoving ? 'active' : 'inactive';
    if (titleEl) {
      titleEl.textContent = activeItem ? activeItem.title : '';
      titleEl.className = 'face-title ' + active;
    }
    if (descEl) {
      descEl.textContent = activeItem ? activeItem.description : '';
  ...
  function start() {
    ...
    if (sketch) {
      sketch._stopped = false;
      sketch.run();
      return;
    }
    const items = window.INFINITE_MENU_ITEMS || defaultItems;
    const scale = window.INFINITE_MENU_SCALE != null ? window.INFINITE_MENU_SCALE : 1.8;
    sketch = new InfiniteGridMenu(
      canvas,
      items.length ? items : defaultItems,
      (index) => {
        const item = items[index % items.length];
        updateOverlay(item, sketch ? sketch.movementActive : false);
      },
      (isMoving) => {
        const index = sketch ? sketch.nearestVertexIndex : null;
        const item = index != null && items.length ? items[index % items.length] : null;
        updateOverlay(item, isMoving);
      },
      (sk) => sk.run(),
      scale
    );
    resizeHandler = () => { if (sketch) sketch.resize(); };
    window.addEventListener('resize', resizeHandler);
  }

  function stop() {
    if (sketch) sketch._stopped = true;
